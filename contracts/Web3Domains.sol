...
contract Web3Domains is ERC721, ERC721Enumerable, AdminControl, RecordStorage, BookingList
{
	...
	address private contractV1 = address(0);
	...
	constructor() ERC721("Trust Web3 Identity", "EVMDID") {
	...
	function getOwnerV1(string memory domain) external view returns (address){
		try IRegisterController(contractV1).getOwner(domain) returns (address result) {
            return result;
        } catch {
            return address(0);
        }
	...
	function setContractV1(address _address) public onlyOwner {
        contractV1 = _address;
    }
	...
	function buyDomain(address to, string memory domain, string memory tld) external payable 
	{
		...
		if (contractV1 != address(0))
		{
			require(!IRegisterController(contractV1).exists(tokenId), "V1 Already exists");
		}
	...
	function reverseOf(address account) public view returns (string memory){
        ...
        require(_isApprovedOrOwner(account, tokenId), "ReverseResolver: not owner nor approved");
        return _tokenURIs[tokenId];
    }
	
	function setReverse(uint256 tokenId) public {
        address _sender = _msgSender();
        require(_isApprovedOrOwner(_sender, tokenId), "ReverseResolver: not owner nor approved");
	...
	function removeReverse() public {
        ...
        require(tokenId != 0, "ReverseResolver: not owner nor approved");
	...
	function _safeMintSubDomain(address to, uint256 tokenId, string memory sub, bytes memory _data) internal {
		...
		require (!_exists(_newTokenId), "Already exists");
